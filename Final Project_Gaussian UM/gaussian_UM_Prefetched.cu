...
int main(int argc, char *argv[])
{
    ...
    // Assuming GPU 0 is used for computations
    int device = 0;
        ...
        if (argv[i][0]=='-') {// flag
            ...
            switch (flag) {
                case 's': // platform
                    ...
                    // Advise the runtime about memory usage
                    cudaMemPrefetchAsync(a, Size * Size * sizeof(float), device, stream1);
                    cudaMemPrefetchAsync(b, Size * sizeof(float), device, stream2);
                    cudaMemPrefetchAsync(m, Size * Size * sizeof(float), device, stream3);
...
void ForwardSub() {
    ...
    for (t=0; t<(Size-1); t++) {
        ...
        Fan1<<<dimGrid, dimBlock, 0, stream1>>>(m, a, Size, t);
        cudaStreamSynchronize(stream1);
        // Fan2<<<dimGridXY,dimBlockXY>>>(m,a,b,Size,Size-t,t);
        // cudaDeviceSynchronize();
        Fan2<<<dimGridXY, dimBlockXY, 0, stream2>>>(m, a, b, Size, Size - t, t);
...
void BackSub()
{
    ...
    cudaMallocManaged(&finalVec, Size * sizeof(float));
    cudaMemPrefetchAsync(finalVec, Size * sizeof(float), device, stream4);
