        ...
        if (argv[i][0]=='-') {// flag
            ...
            switch (flag) {
                case 's': // platform
                    ...
                    cudaMemPrefetchAsync(a, Size * Size * sizeof(float), device, nullptr);
                    cudaMemPrefetchAsync(b, Size * sizeof(float), device, nullptr);
                    cudaMemPrefetchAsync(m, Size * Size * sizeof(float), device, nullptr);
...
void BackSub()
{
    ...
    cudaMemPrefetchAsync(finalVec, Size * sizeof(float), device, nullptr);
