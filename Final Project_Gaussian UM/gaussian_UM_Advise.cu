        ...
        if (argv[i][0]=='-') {// flag
            ...
            switch (flag) {
                case 's': // platform
                    ...
                    cudaMemAdvise(a, Size * Size * sizeof(float), cudaMemAdviseSetPreferredLocation, device);
                    cudaMemAdvise(b, Size * sizeof(float), cudaMemAdviseSetPreferredLocation, device);
                    cudaMemAdvise(m, Size * Size * sizeof(float), cudaMemAdviseSetPreferredLocation, device);
...
void BackSub()
{
    ...
    cudaMemAdvise(finalVec, Size * sizeof(float), cudaMemAdviseSetPreferredLocation, device);
